...
class TFAdaptiveSoftmaxMask(tf.keras.layers.Layer):
    ...
    def call(self, hidden, target, return_mean=True, training=False):
        ...
        if self.n_clusters == 0:
            ...
            # print('self.cutoffs',self.cutoffs)
            for i in range(len(self.cutoffs)):
                l_idx, r_idx = self.cutoff_ends[i], self.cutoff_ends[i + 1] # 0 20000 40000 200000 config vocabsize
                if target is not None:
                    mask = (target >= l_idx) & (target < r_idx) # ex target의 요소중에  0 <= 요소 < 20000 만족하는 값에 대해서 전부 True 반환
                    mask_idx = tf.where(mask)
                    cur_target = tf.boolean_mask(target, mask) - l_idx

                    '''
                    cond_tensor = tf.constant([[True, False, True],
                           [False, True, False],
                           [True, True, False]])

                    # tf.where를 사용하여 조건을 만족하는 위치 찾기
                    indices = tf.where(cond_tensor)
                    [[0 0] 0열 0행 에 있다. 
                    [0 2]
                    [1 1]
                    [2 0]
                    ...
                    # 불리언 마스크 생성 (예: 짝수인 요소를 선택)
                    mask = original_tensor % 2 == 0

                    # tf.boolean_mask를 사용하여 조건을 만족하는 요소 선택
                    filtered_tensor = tf.boolean_mask(original_tensor, mask)

                    # 결과 출력
                    print("원본 텐서:\n", original_tensor.numpy())
                    print("불리언 마스크:\n", mask.numpy())
                    print("조건을 만족하는 요소들:\n", filtered_tensor.numpy())
                ...
                if self.div_val == 1:
                    cur_W = self.out_layers[0][0][l_idx:r_idx] # ex)0 ~ 20000 까지를 보겠다
                ...
                if i == 0:
                    ...
                    out.append(head_logprob[..., : self.cutoffs[0]]) # 3차원 배열에서 전면이 열과 행이라고 했을 때 뒤의 3차원에 대해 0이면 가장 전면을 1이면 두번째 면을 선택하는 것이라고 이해하자
                    if target is not None:
